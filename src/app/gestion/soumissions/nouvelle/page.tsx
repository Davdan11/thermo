...
/* ?client=<id> (fiche client) : les coordonnées du client sont les valeurs initiales du créateur (rendues au serveur).
   ...
   change. QuoteBuilder n'est pas modifié : il charge déjà les jumelages de la machine initiale. */
export default async function NouvellePage({ searchParams }: { searchParams: Promise<{ client?: string; thermomatch?: string; choix?: string }> }) {
  ...
  const [settings, pre] = await Promise.all([readSettings(), clientId ? clientPrefill(clientId) : Promise.resolve(null)]);
  const today = todayIn();
  ...
  const builder = <QuoteBuilder quoteId={null} number={null} version={1} initial={initial} internalNotes="" settings={settings} today={today} rates={CURRENT_RATES} />;
  return tm ? (
    <>
