    ...
    >
      ...
      {card.price && (
        <div className="mx-6 mt-4 rounded-[18px] px-4 py-3.5 sm:mx-7" style={{ border: `1px solid ${C.inkLine}` }}>
          ...
          {/* L'aide LogisVert est versée au client par Hydro-Québec : jamais soustraite de notre prix, jamais garantie. */}
          {card.subsidy > 0 && (
            <p className="text-[12.5px] leading-snug" style={{ color: C.inkMute, margin: "4px 0 0" }}>
              Aide LogisVert à part : <strong className="tabular-nums" style={{ color: C.green }}>{fr(card.subsidy)} $</strong> versés au client par Hydro-Québec (montant officiel pour cet appariement, à faire confirmer).
            </p>
          )}
          ...
          <div>
            ...
            <ul className="mt-3 space-y-1.5" style={{ listStyle: "none", padding: 0 }}>
              ...
              <li className="pt-1 font-semibold" style={{ color: C.cream }}>
                = {fr(Math.round(ctx.estimatedLoadBtu))} BTU/h à −15 °C{typeof ctx.uncertaintyPct === "number" ? ` (±${ctx.uncertaintyPct} %)` : ""}
              </li>
            </ul>
          </div>
        )}
          ...
          <div>
            ...
            {typeof ctx.candidatesEvaluated === "number" && (
              <p style={{ margin: "14px 0 0" }}>
                {fr(ctx.candidatesEvaluated)} machines évaluées, {typeof ctx.candidatesRetained === "number" ? `${fr(ctx.candidatesRetained)} de calibre compatible, ` : ""}{kept} retenue{kept > 1 ? "s" : ""}.
