---
title: "Installer une thermopompe en hiver au Québec : possible, et souvent plus rapide"
description: "Ce que le froid change réellement à l'installation d'une murale ou d'une centrale, les limites de température des fabricants, les délais plus courts hors saison, et les vérifications à exiger d'un installateur qui pose en janvier."
author: "L'équipe ThermoMatch"
publishedAt: "2026-09-11"
updatedAt: "2026-09-11"
coverImage: "/images/about-experience-house.jpg"
category: "installation"
keywords: ["installer thermopompe hiver", "installation thermopompe janvier", "thermopompe installation froid", "délai installation thermopompe", "installer thermopompe décembre"]
related: ["preparer-son-projet-installation", "entretien-thermopompe-hiver", "thermopompe-grand-froid-moins-30", "subvention-logisvert-guide-complet"]
faq:
  - q: "Peut-on installer une thermopompe à -20 °C?"
    a: "L'installation mécanique et électrique, oui. La mise en service (tirage au vide, charge de réfrigérant, essai) a des limites : la plupart des fabricants demandent une température extérieure minimale pour l'essai en mode chauffage ou climatisation. L'installateur planifie ces étapes une journée plus douce ou reporte l'essai final, sans bloquer le chauffage."
  - q: "Les délais sont-ils plus courts en hiver?"
    a: "En général, oui. Le pic des installations va d'avril à septembre, tiré par la climatisation. De novembre à mars, les carnets se libèrent, les délais passent souvent de plusieurs semaines à quelques jours, et certains installateurs ajustent leurs prix hors saison."
  - q: "La subvention LogisVert change-t-elle selon la saison?"
    a: "Non. Le montant dépend de l'appariement installé et de la liste en vigueur au moment de la demande. Installer en hiver permet en revanche de profiter de la thermopompe sur la saison de chauffage en cours au lieu d'attendre l'automne suivant."
  - q: "Quels risques spécifiques à l'hiver?"
    a: "Un socle posé sur un sol gelé qui bougera au dégel, une unité extérieure trop basse par rapport à l'accumulation de neige, un tirage au vide bâclé parce qu'il fait froid, et une charge de réfrigérant approximative. Ce sont des questions de rigueur, pas de saison : exigez le rapport de tirage au vide et la hauteur d'installation par écrit."
---

L'idée qu'une thermopompe s'installe entre mai et septembre vient de la climatisation. Pour le chauffage, l'hiver est au contraire une bonne saison : les installateurs sont disponibles, les délais sont courts, et la machine commence à réduire la facture dès la semaine suivante. À condition de connaître les quelques limites réelles que le froid impose.

## Ce que le froid change, et ce qu'il ne change pas

**Ne change pas :** le percement du mur, la pose du support ou du socle, le passage de la tuyauterie de cuivre, le câblage, le raccordement du condensat. Tout cela se fait à toute température, avec des gants.

**Change :** la mise en service. Trois étapes sont sensibles au froid.

1. **Le tirage au vide.** Il élimine l'humidité et l'air des conduites avant la charge. Par grand froid, l'humidité résiduelle gèle et fausse la lecture ; le tirage doit durer plus longtemps et être vérifié par un test de maintien (le vide doit tenir sans remonter). Demandez la valeur atteinte en microns et la durée du maintien.
2. **La charge de réfrigérant.** Les machines sont livrées préchargées pour une longueur de tuyauterie standard ; au-delà, l'installateur ajoute du réfrigérant au poids. Le froid rend la lecture des pressions moins parlante ; la charge au poids, avec une balance, reste la seule méthode fiable.
3. **L'essai de fonctionnement.** Les fabricants fixent une plage de température extérieure pour l'essai en mode climatisation (souvent au-dessus de -5 à 0 °C) et parfois pour l'essai en chauffage. En janvier, l'installateur teste le chauffage, note les paramètres, et revient au printemps pour l'essai en climatisation si le fabricant l'exige. Cela doit être écrit sur la facture.

## Les vérifications propres à l'hiver

**La hauteur de l'unité extérieure.** La neige s'accumule ; le dégivrage produit de l'eau qui gèle au sol. L'unité doit être surélevée (support mural ou socle) de façon que le bas reste dégagé de l'accumulation habituelle de votre région, 30 à 60 cm au minimum, davantage au Saguenay ou en Abitibi. Un support fixé sur le mur évite le problème du socle qui bouge au dégel.

**Le drainage du dégivrage.** L'eau du dégivrage doit s'écouler sans former de glace sous l'unité ni sur une allée. Sur certaines installations exposées, un câble chauffant dans le bac est recommandé par le fabricant.

**L'emplacement par rapport aux vents dominants et aux toits.** Une unité sous une bordure de toit reçoit la neige et la glace qui glissent. Une unité face aux vents dominants se remplit de neige poudreuse. Ce sont les deux causes les plus fréquentes d'appels de service en février.

**L'appoint pendant les travaux.** Si la thermopompe remplace une fournaise, la maison doit rester chauffée pendant le remplacement ; l'installateur planifie la coupure sur quelques heures en journée. Avec des plinthes, il n'y a pas d'interruption.

## Les avantages concrets de l'hiver

- **Délais.** Hors saison, un projet se réalise souvent en une à deux semaines au lieu de quatre à huit.
- **Disponibilité des meilleurs installateurs**, ceux qui refusent du travail l'été.
- **Économies immédiates** sur la saison de chauffage en cours, la plus coûteuse de l'année.
- **Vérification en conditions réelles** : vous voyez tout de suite si la machine tient la maison à -15 °C, ce qu'une installation de juin ne permet pas de constater avant décembre.

## Ce qu'il faut exiger de l'installateur

1. La licence RBQ et, pour la partie électrique, un maître électricien.
2. Le rapport de tirage au vide (microns atteints, durée de maintien) et la quantité de réfrigérant ajoutée, au poids.
3. La hauteur d'installation de l'unité extérieure et le mode de drainage du dégivrage, par écrit.
4. La date prévue de l'essai en climatisation si le fabricant l'impose au printemps.
5. Les numéros exacts de l'unité extérieure et de l'unité intérieure, pour la demande LogisVert.

Le questionnaire ThermoMatch retient les machines certifiées climat froid adaptées à votre maison ; l'installateur partenaire de votre région vous remet ensuite le prix écrit et propose une date, souvent dans la semaine en hiver.
