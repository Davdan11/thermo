...
export async function askCrmAssistant(input: AskInput, deps: AskDeps = {}): Promise<AskResult> {
  ...
  try {
    ...
    const call =
      deps.call ??
      (mode === "api"
        ? createAnthropicCaller({ apiKey: env.ANTHROPIC_API_KEY!.trim(), fetchImpl: deps.fetchImpl, timeoutMs: L.timeoutMs, betas: usesServerFallbacks(model) ? [FALLBACK_BETA] : [] })
    ...
    try {
      result = await runToolLoop({
        ...
        fallbacks: mode === "api" && usesServerFallbacks(model),
        onUsage: (u) => {
