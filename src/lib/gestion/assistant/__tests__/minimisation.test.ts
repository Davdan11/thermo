...
describe("ce qui part vers Anthropic", () => {
  ...
  it("appel direct : bonne adresse, en-têtes, modèle par défaut claude-opus-5, repli côté serveur éteint par défaut", async () => {
    ...
    expect(c.headers).not.toHaveProperty("anthropic-beta");
    expect(c.body.model).toBe("claude-opus-5");
    expect(c.body).not.toHaveProperty("fallbacks");
    ...
    // Allumé explicitement : en-tête bêta et paramètre « fallbacks » pour le modèle par défaut.
    const h = scriptedFetch([reply([text("Bonjour.")], "end_turn")]);
    await askCrmAssistant({ email: "proprio@exemple.ca", question: "Bonjour" }, { env: { ...PROD, ANTHROPIC_SERVER_FALLBACKS: "1" }, fetchImpl: h.fetch, context: ctx, now: NOW });
    expect(h.calls[0].headers["anthropic-beta"]).toBe(FALLBACK_BETA);
    expect(h.calls[0].body.fallbacks).toBe("default");
  });
