...
export function partnerAutoTasks(input: PartnerTaskInput | undefined, now: Date): Task[] {
  ...
  for (const inst of input.installers) {
    ...
    for (const kind of ["rbq", "assurance"] as const) {
  ...
  for (const t of sav.tickets) {
    ...
    if (t.status === "assigne" && isLate(t, now)) out.push(mk("sav-billet", `${t.id}:retard:${t.dueAt}`, { title: `Billet n° ${t.number} : prise en charge en retard`, detail: "Aucune visite prévue à l’échéance", dueAt: t.dueAt, ...base }));
